...
//cl::Buffer buffer_init(cl::Context ctx, ::size_t size, short a)
//{
//	cl::Buffer new_buff(ctx, CL_MEM_READ_WRITE, size);
//	return new_buff;
//}
...
int main() 
{
	...
	int count = 0;
	short offset_list[0.5*(n/128)*((n/128)+1)]; 
	for (int p = 0; p<(n/128); p++)
	{
		for (int q = 0; q<p; q++)
		{
			offset_list[count] = offset_list[count-1] + 128*128;
        		for (int s = 0; s<128; s++)
			{
            			for (int t = 0; t<128; t++)
				{
		...
		{    		
			if (count == 0) { offset_list[count] = 0; }
			else { offset_list[count] = offset_list[count - 1] + 64*127; }
			el			
	...
	cl::Buffer l2buff(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, ::size_t(2*n_el), l2);
	cl::Buffer l1buff(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, ::size_t(2*n_el), l1);
	...
	std::vector<cl::Buffer> offsets = { buffer_init(ctx,::size_t (2), short (0)) };
	...
	for (int i = 1; i<(pow(n,2)/pow(128,2)); i++)
	{
		...
		Vincbuff += { buffer init(ctx,::size_t (8*128*128)) };
		Intincbuff += { buffer init(ctx,::size_t (8*128*128)) };		
		
	}
	for (int i = 1; i<0.5*(n/128)*((n/128)+1); i++)
	{
		offsets += { buffer_init(ctx, ::size_t (2), offset_list[i] };
	...
	//	ker_F[i].setArg(10, vincbuff[i]);
	//	ker_F[i].setArg(11, wincbuff[i]);
	//	ker_F[i].setArg(12, Vincbuff);
	//	ker_F[i].setArg(13, Intincbuff);
	//	ker_F[i].setArg(14, Ftmp);
	//	ker_F[i].setArg(15, nbuff);
	//}
	...
	ker_S.setArg(9, Vincbuff);
	ker_S.setArg(10, Intbuff);
	ker_S.setArg(11, Intincbuff);

	ker_Sp.setArg(0, vincbuff[0]);
	ker_Sp.setArg(1, wincbuff[0]);
	ker_Sp.setArg(2, accelbuff);
	ker_Sp.setArg(3, alphabuff);
	ker_Sp.setArg(4, mbuff);
	ker_Sp.setArg(5, Ibuff);
	ker_Sp.setArg(6, radbuff);
	ker_Sp.setArg(7, nbuff);
	ker_Sp.setArg(8, Vbuff);
	ker_Sp.setArg(9, Vincbuff);
	ker_Sp.setArg(10, Intbuff);
	ker_Sp.setArg(11, Intincbuff);
