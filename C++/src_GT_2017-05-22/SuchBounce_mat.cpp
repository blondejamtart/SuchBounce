...
template <typename T>
std::vector<T> &operator+=(std::vector<T> &A, const std::vector<T> &B)	// defines "+=" for std::vectors as concatenation
{
...
int main() 
{
	...
	cl::Kernel ker_comms = kernel_init("logic_comms.cl", "comms", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_interact = kernel_init("logic_interact.cl", "interact", ctx, ctxDevices, OpenCL_log);	
	...
	std::vector<cl::Buffer> pheremoneincbuff = { buffer_init(ctx,::size_t (8*n_block[0]*n_block[0]*4)) };
	...
	for (int i = 1; i<(pow(n,2)/pow(n_block[0],2)); i++)
	{
		...
		pheremoneincbuff += { buffer_init(ctx,::size_t (8*n_block[0]*n_block[0])) };	
	}
	...
	cl::Buffer pheremonesumbuff(ctx, CL_MEM_READ_WRITE, ::size_t (8*n*n/n_block[0]));
	...
	ker_S.setArg(12, pheremonebuff);
	ker_S.setArg(13, pheremoneincbuff[0]);	
	...
	ker_S_interm.setArg(10, pheremonesumbuff);	
	...
	ker_comms.setArg(5, rangebuff);
	ker_comms.setArg(6, oppbuff);
	ker_comms.setArg(7, genes);
	ker_comms.setArg(8, pheremonebuff);
...
//			mu_tracker << tempstring;
		...
		for (int a = 0; a < (n/n_block[0]); a++)
		{
			...
			{													
				...
				ker_Sp.setArg(5, pheremoneincbuff[a*(n/n_block[0])+b]);
		...
		{	
			...
			if (a != b)
			{
				...
				ker_S_interm.setArg(11, pheremoneincbuff[a*(n/n_block[0])+b]);			
