...
%R_encounter = 1.2e5;
theta = 0; %:0.125*pi:pi;
for R_encounter = 1.2e5; %7*6.371e6:-6.371e6:6.371e6    
    
    [r_offset, v_offset, settings, settings_length] = binary_orbit_find(R_encounter,8.0e3);
    r_temp = zeros(1,n,3);
    v_temp = zeros(1,n,3);
   ...
   settings{29} = ['int block_size = ' num2str(256) '; // Size of blocks for partitioning of particle interaction calculations '];
    ...
    for i = 1:size(theta,2)
        root = ['../../binary_' num2str(n) '_theta_' num2str(theta(i)) '_NR/'];
%         root = ['/media/falcon/Tesla_Data/binary_' num2str(n) '_' num2str(R_encounter) '/'];
        ...
        filewrite([root '/Setup/m.vec'],[1e-2*ones(1,n-2) 1e4*(100*sqrt(n/1024))^3 1e4*(100*sqrt(n/1024))^3],'init')
        filewrite([root '/Setup/rad.vec'],[1e-2*ones(1,n-2) 100*sqrt(n/1024) 100*sqrt(n/1024)],'init')
        r = random_seed(n,[1e-2*ones(1,n-2) 100*sqrt(n/1024) 100*sqrt(n/1024)]);
...
%         r_temp0 = fileread('../Outputs/r_final.dat');
        r_temp0 = r;
        [v_temp0,r_temp0] = spinup(r_temp0,[1e-2*ones(1,n-2) 1e4*(100*sqrt(n/1024))^3 1e4*(100*sqrt(n/1024))^3],[1e-2*ones(1,n-2) 100*sqrt(n/1024) 100*sqrt(n/1024)],n,theta(i));
    ...
    settings{18} = ['const double settings[14] = {pow(2,16), pow(2.0,-3), pow(2.0,-3), '...
        '0.0e-02, 2.5e-15, 0, 0.4, 0.3, 5.0e-03, pow(2.0,8), 0.0e-18, 1.0e-03, 0.0e+04, 0.0e-02};'];
    settingsfile = fopen('../Settings.h','w');
    for j = 1:settings_length
        fprintf(settingsfile,'%s\n',settings{j});
    end
...
%         root = ['/media/falcon/Tesla_Data/binary_' num2str(n) '_' num2str(R_encounter) '/'];
        ...
        filewrite([root 'm.vec'],[1e4*(100*sqrt(n/1024))^3 1e4*(100*sqrt(n/1024))^3 5.972e24],'init')
        filewrite([root 'rad.vec'],[100*sqrt(n/1024) 100*sqrt(n/1024) 6.371e6],'init')
        ...
        filewrite(['../Setup/' 'm.vec'],[1e4*(100*sqrt(n/1024))^3 1e4*(100*sqrt(n/1024))^3 5.972e24],'init')
        filewrite(['../Setup/' 'rad.vec'],[100*sqrt(n/1024) 100*sqrt(n/1024) 6.371e6],'init')
