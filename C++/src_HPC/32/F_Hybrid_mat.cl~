...
//((1.0/6.0)*stuff[2]*cut_off*(fpe-fme-2*rad_a*rad_b*(pow(fpe,2)+pow(fme,2))))*(pow((d-hard_rad),2)/(cut_off-hard_rad) - (cut_off-hard_rad)) + (1.0/6.0)*stuff[2]*(2*rad_a*rad_b*(fpe+fme)-log(fpe)+log(fme));
			...
			float j = (F*t_step+0.5f*dF*t_step*t_step)-m_a*m_b/(m_a+m_b)*(collisionflag*(stuff[1]*p0*(t_step-dt)+stuff[8]*(rad_a+rad_b-d0)*(t_step-dt)+0.5f*stuff[8]*p0*(t_step-dt)*(t_step-dt))-(collisionflag-1)*(stuff[1]*p0*dt+0.5f*stuff[8]*p0*dt*dt));
			...
			if (isinf(1.0f/length(v_rel)) != 1) { friction = collisionflag*step(0,jf)*jf*normalize(v_rel); }
			
			int a_sub = a - n[4]*(a/n[4]);
			int b_sub = b - n[4]*(b/n[4]);
			accel_part_lower[a_sub*n[4]+b_sub] = -(j*Runit - friction); 
			alpha_part_lower[a_sub*n[4]+b_sub] = cross(Runit,step(0,jf)*jf*v_rel);			
			
			accel_part_upper[b_sub*n[4]+a_sub] = (j*Runit - friction); 
			alpha_part_upper[b_sub*n[4]+a_sub] = cross(Runit,step(0,jf)*jf*v_rel);	

			Ipart_lower[a_sub*n[4]+b_sub] = 0.25f*collisionflag*m_a*m_b/(m_a+m_b)*stuff[8]*pow((rad_a+rad_b-d0),2);
			Vpart_lower[a_sub*n[4]+b_sub] = Vtemp;	
			
			Ipart_upper[b_sub*n[4]+a_sub] = 0.25f*collisionflag*m_a*m_b/(m_a+m_b)*stuff[8]*pow((rad_a+rad_b-d0),2);
