...
cl::Buffer buffer_init(cl::Context ctx, ::size_t size, double* a)
{
	cl::Buffer new_buff(ctx, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, size, &a);
	//if (sizeof(a) != size) {std::cout << "You done gone mucked up!\n";}
	return new_buff;
...
int main() 
{
	...
	cl::Buffer Ftmp(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, ::size_t (32), onetemp);
	...
	queue[0].enqueueReadBuffer(tbuff, CL_TRUE, ::size_t(0), sizeof(stuff), stuff);
	log_out << "(" << stuff[10] << "); ";	
	queue[0].enqueueNDRangeKernel(ker_scale,offset,local_size,local_size); // Ensure current timestep does not exceed maximum
	queue[0].enqueueReadBuffer(tbuff, CL_TRUE, ::size_t(0), sizeof(stuff), stuff);	
	log_out << "(" << stuff[10] << ")\n";		
	double v_norm[n] = { 0 };
	...
	queue[0].enqueueReadBuffer(Ftmp, CL_TRUE, ::size_t (0), ::size_t (32), &F1);
	...
	for(int init_x=0; init_x<t_test; init_x ++)
	{
		...
		//std::cout << stuff[0] << ", ";
		t_now += stuff[0];
		queue[0].enqueueNDRangeKernel(ker_scale,offset,local_size,local_size); 	// Set new time step
