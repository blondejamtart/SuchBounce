...
void string2array(std::string str, double* arr, int length)			// converts csv string to double array
{
...
int main() 
{
	...
	int count[2] = { 0 };
	auto offsets_0 = new int[((n / n_block[0])*((n / n_block[0]) + 1))/2];
	auto offsets_1 = new int[int(pow(n, 2) / pow(n_block[0], 2))];
	offsets_0[0] = 0;
	for (int p0 = 0; p0<(n/n_block[0]); p0++)
	{
		...
		l3[count[1]] = p0; l4[count[1]] = p0;
		count[1]++;
		for (int i=0; i<(n_block[1]*n_block[2]); i++)
		{    		
							
			l1[count[0]] = l1_block0[i] + n_block[0]*p0;
    		l2[count[0]] = l2_block0[i] + n_block[0]*p0;
    		count[0]++;
		}
		for (int u = 0; u<(n/n_block[0]); u++)
		{
			offsets_1[(n/n_block[0])*p0+u] = n*p0+u;
		}    		
	}
	...
	// Build Kernels	
	std::vector<cl::Kernel> ker_F = { kernel_init("src_windows\\F_Hybrid_mat.cl", "Fimp", ctx, ctxDevices, OpenCL_log) };
	std::vector<cl::Kernel> ker_Sp = { kernel_init("src_windows\\reduce_pairwise_t.cl", "red_pair", ctx, ctxDevices, OpenCL_log) };
	std::vector<cl::Kernel> ker_S_interm = { kernel_init("src_windows\\reduce_mat_interm.cl", "red_copy", ctx, ctxDevices, OpenCL_log) };

	cl::Kernel ker_S_copy = kernel_init("src_windows\\reduce_MD_devcopy.cl", "red_copy", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_Sp0 = kernel_init("src_windows\\reduce_pairwise.cl", "red_pair", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_T = kernel_init("src_windows\\kinetic.cl", "Tstep", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_r = kernel_init("src_windows\\position.cl", "rstep", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_S = kernel_init("src_windows\\reduce_mat.cl", "red", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_t = kernel_init("src_windows\\translate.cl", "rmove", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_t0 = kernel_init("src_windows\\translate_0.cl", "rmove0", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_t_mean = kernel_init("src_windows\\translate_mean.cl", "rmove", ctx, ctxDevices, OpenCL_log);
    cl::Kernel ker_v_0 = kernel_init("src_windows\\velocity_mat.cl", "vstep", ctx, ctxDevices, OpenCL_log);
   	cl::Kernel ker_v_1 = kernel_init("src_windows\\velocity_mat.cl", "vstep", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_0_0 = kernel_init("src_windows\\zero.cl", "zeroer", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_0_1 = kernel_init("src_windows\\zero.cl", "zeroer", ctx, ctxDevices, OpenCL_log);
	cl::Kernel ker_0_2 = kernel_init("src_windows\\zero_vec.cl", "zeroer", ctx, ctxDevices, OpenCL_log);
	...
	std::vector<cl::Buffer> accelsumbuff = { buffer_init(ctx, ::size_t(8 * n*n / n_block[0] * 4)) };
	std::vector<cl::Buffer> alphasumbuff = { buffer_init(ctx, ::size_t(8 * n*n / n_block[0] * 4)) };
	std::vector<cl::Buffer> Vsumbuff = { buffer_init(ctx, ::size_t(8 * n*n / n_block[0])) };
	std::vector<cl::Buffer> Intsumbuff = { buffer_init(ctx, ::size_t(8 * n*n / n_block[0])) };
	...
	for (int i = 1; i < nDevs; i++)
	{
		...
		accelsumbuff += { buffer_init(ctx, ::size_t(8 * n*n / n_block[0] * 4)) };
		alphasumbuff += { buffer_init(ctx, ::size_t(8 * n*n / n_block[0] * 4)) };
		Vsumbuff += { buffer_init(ctx, ::size_t(8 * n*n / n_block[0])) };
		Intsumbuff += { buffer_init(ctx, ::size_t(8 * n*n / n_block[0])) };

		tbuff += { buffer_init(ctx, sizeof(stuff), stuff) };
		Fbuff += { buffer_init(ctx, ::size_t (8*n_el)) };		
		
		ker_Sp += { kernel_init("src_windows\\reduce_pairwise_t.cl", "red_pair", ctx, ctxDevices, OpenCL_log) };
		ker_S_interm += { kernel_init("src_windows\\reduce_mat_interm.cl", "red_copy", ctx, ctxDevices, OpenCL_log) };
		ker_F += { kernel_init("src_windows\\F_Hybrid_mat.cl", "Fimp", ctx, ctxDevices, OpenCL_log) };
	}


	auto manyones = new double[n_block[0] * n_block[0]];
	auto queueflags = new short[nDevs*(n/n_block[0])*(n/n_block[0])];
	for (int i = 0; i < 0.5*(n / n_block[0])*((n / n_block[0]) + 1); i++) 				// Loop over all interaction blocks
	{
		...
		{
			if (j == ind) { queueflags[j*(n / n_block[0])*(n / n_block[0]) + a*(n / n_block[0]) + b] = 1; queueflags[j*(n / n_block[0])*(n / n_block[0]) + b*(n / n_block[0]) + a] = 1; }
			else { queueflags[j*(n / n_block[0])*(n / n_block[0]) + a*(n / n_block[0]) + b] = 0; queueflags[j*(n / n_block[0])*(n / n_block[0]) + b*(n / n_block[0]) + a] = 0; }
		}
		
	}

	for (int i = 0; i < (n_block[0] * n_block[0]); i++) { manyones[i] = 1.0; }
	...
	for (int i = 1; i<(pow(n,2)/pow(n_block[0],2)); i++)
	{
		...
		Vincbuff += { buffer_init(ctx,::size_t (8*n_block[0]*n_block[0]), manyones) };
		Intincbuff += { buffer_init(ctx,::size_t (8*n_block[0]*n_block[0]), manyones) };
	}
	...
	for(int i = 0; i < nDevs; i++)	
	{
		...
		ker_S_interm[i].setArg(2, accelsumbuff[i]);
		ker_S_interm[i].setArg(3, alphasumbuff[i]);
		ker_S_interm[i].setArg(4, nbuff[i]);
		ker_S_interm[i].setArg(5, Vsumbuff[i]);
		ker_S_interm[i].setArg(7, Intsumbuff[i]);	
	}
	
	ker_Sp0.setArg(0, accelsumbuff[0]);
	ker_Sp0.setArg(1, alphasumbuff[0]);
	ker_Sp0.setArg(2, nbuff[0]);
	ker_Sp0.setArg(3, Vsumbuff[0]);
	ker_Sp0.setArg(4, Intsumbuff[0]);	

	ker_S.setArg(0, accelsumbuff[0]);
	ker_S.setArg(1, alphasumbuff[0]);		
	...
	ker_S.setArg(9, Vsumbuff[0]);
	ker_S.setArg(10, Intbuff);
	ker_S.setArg(11, Intsumbuff[0]);	

	ker_S_copy.setArg(2, accelsumbuff[0]);
	ker_S_copy.setArg(3, alphasumbuff[0]);
	ker_S_copy.setArg(4, nbuff[0]);
	ker_S_copy.setArg(5, Vsumbuff[0]);
	ker_S_copy.setArg(7, Intsumbuff[0]);
...
//		queue[0].enqueueReadBuffer(rbuff[0], CL_TRUE, ::size_t (0), vecsize, r);
		...
		for (int i = 0; i < 0.5*(n / n_block[0])*((n / n_block[0]) + 1); i++) 				// Loop over all interaction blocks
		{
			...
			//queue[ind].enqueueWriteBuffer(Vincbuff[b*(n / n_block[0]) + a], CL_TRUE, ::size_t(0), ::size_t(8*n_block[0]*n_block[0]), manyones);
			//queue[ind].enqueueWriteBuffer(Vincbuff[a*(n / n_block[0]) + b], CL_TRUE, ::size_t(0), ::size_t(8 * n_block[0] * n_block[0]), manyones);
		}	
		...
		for (int i = 0; i < 0.5*(n / n_block[0])*(n / n_block[0] + 1); i++) 														// Perform pairwise sum of all interaction quantities for each interaction block
		{
			...
			int ind = i - nDevs*floor(i / nDevs);																																						// |
			queue[0].flush();
			ker_S_copy.setArg(0, accelsumbuff[ind]);																					// |
			ker_S_copy.setArg(1, alphasumbuff[ind]);																					// |
			ker_S_copy.setArg(6, Vsumbuff[ind]);																					// |
			ker_S_copy.setArg(8, Intsumbuff[ind]);

			ker_S_copy.setArg(9, offset_buff_1[a*(n / n_block[0]) + b]);																				// |
		
			queue[0].enqueueNDRangeKernel(ker_S_copy, offset, gsizeRed, local_size);
			if (a != b)
			{
				queue[0].flush();
				ker_S_copy.setArg(9, offset_buff_1[b*(n / n_block[0]) + a]);																		// |
				queue[0].enqueueNDRangeKernel(ker_S_copy, offset, gsizeRed, local_size);
			}
		}
	...
	while (t_now < max_time && t_now >= 0)
	{
		...
		for (int i = 0; i < 0.5*(n / n_block[0])*((n / n_block[0]) + 1); i++) 				// Loop over all interaction blocks
		{
			...
			//queue[i].enqueueWriteBuffer(Intincbuff[b*(n / n_block[0]) + a], CL_TRUE, ::size_t(0), ::size_t(8*n_block[0]*n_block[0]), manyones);
			//queue[i].enqueueWriteBuffer(Intincbuff[a*(n / n_block[0]) + b], CL_TRUE, ::size_t(0), ::size_t(8 * n_block[0] * n_block[0]), manyones);
