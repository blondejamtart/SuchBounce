...
__kernel void Fimp(__global double *q,			
		...
		{ 			
			...
			VCpart[x] = q_a*q_b*e0/d;			
			double 	F = -Vpart[x]/d; //(((m_a*m_b*G) - (q_a*q_b*e0))/(d*d));
			double 	dF = -2*p*Vpart[x]/pow(d,2); //((-2*(m_a*m_b*G) + 2*(q_a*q_b*e0))/(d*d*d))*p;	
...
//((1.0/6.0)*stuff[2]*cut_off*(fpe-fme-2*rad_a*rad_b*(pow(fpe,2)+pow(fme,2))))*(pow((d-hard_rad),2)/(cut_off-hard_rad) - (cut_off-hard_rad)) + (1.0/6.0)*stuff[2]*(2*rad_a*rad_b*(fpe+fme)-log(fpe)+log(fme));
			...
			thermpart[x] = m_a*m_b/(m_a+m_b)*stuff[1]*(fabs(collisionflag*pow(p0,2)*(t_step-dt)) + fabs((collisionflag-1)*pow(p0,2)*dt)) + fabs(0.5*jf*length(v_rel));  
