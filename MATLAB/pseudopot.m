...
ii = 1000;
for i = 1:ii
    d(i) = 0.9*(rad(1)+rad(2))+i*1e-7;
    fplus = 1/((d(i)^2) - ((rad(1)+rad(2))^2));
    fminus = 1/((d(i)^2) - ((rad(1)-rad(2))^2));
    
     
     
    if d(i) < d_e+1e-8
        V_p1(i) = (1.0/3.0)*A*((1.0/2.0)*(2*rad(1)*rad(2)*(fpe+fme)+log(fpe)-log(fme))+d_e*(d(i)-d_e)*(fpe-fme-2*rad(1)*rad(2)*...
          ((fpe^2)+(fme^2)))+0.5*((d(i)-d_e)^2)*((fpe-fme-2*rad(1)*rad(2)*((fpe^2)+(fme^2)))+2*(d_e^2)*(2*rad(1)*rad(2)*((fpe^3)+(fme^3))...
            +(fme^2)-(fpe^2))));
         
        V_p2(i) = V_d_a*0.5*((d(i)-0.98*(rad(1)+rad(2)))^2/(d_e-0.98*(rad(1)+rad(2))) - (d_e-0.98*(rad(1)+rad(2)))) + V_a;
...
plot(d-2*rad(1),V_p2,d-2*rad(1),V_t)