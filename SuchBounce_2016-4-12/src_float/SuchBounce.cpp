...
int main() 
{
	...
	cl::Buffer vincbuff(ctx, CL_MEM_READ_WRITE, ::size_t (4*n_el*4));
	cl::Buffer wincbuff(ctx, CL_MEM_READ_WRITE, ::size_t (4*n_el*4));
	...
	for(int init_x=0; init_x<2048; init_x ++)
	{
		...
		if (( t_now == 0 || (t_now - t_last) >= (1.0 / 64.0)*warp) && framecount < n_frames)
		{
			...
			queue.enqueueReadBuffer(Vbuff, CL_TRUE, ::size_t (0), ::size_t(4*n), E_temp);
			tempstring = arraytostring(E_temp, n);
			V_tracker << tempstring;

			queue.enqueueReadBuffer(Intbuff, CL_TRUE, ::size_t (0), ::size_t(4*n), E_temp);
			tempstring = arraytostring(E_temp, n);
			E_tracker << tempstring;

			queue.enqueueReadBuffer(Tvbuff, CL_TRUE, ::size_t (0), ::size_t(4*n), E_temp);
			tempstring = arraytostring(E_temp, n);
			Tv_tracker << tempstring;
		
			queue.enqueueReadBuffer(Twbuff, CL_TRUE, ::size_t (0), ::size_t(4*n), E_temp);
			tempstring = arraytostring(E_temp, n);
