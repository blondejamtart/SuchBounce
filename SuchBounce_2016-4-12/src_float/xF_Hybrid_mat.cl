...
__kernel void Fimp(__global const float *q,			
		...
		{ 			
			...
			float d0 = d; //distance((r_a-v_a*0.5*t_step),(r_b-v_b*0.5*t_step));
		 	float3 Runit = r_a; //normalize(r_a - r_b);			
			float3 wvec = w_a; //w[a]*rad_a + w[b]*rad_b;
			float3 vtemp = v_b; // - v_a; 
			float p = dot(vtemp,Runit);
			float p0 = p; //dot(vtemp,normalize((r_a-v_a*0.5*t_step)-(r_b-v_b*0.5*t_step)));		
			float collisionflag = step(d0,(rad_a+rad_b));

			float cut_off = (rad_a+rad_b) + 0.01*min(rad_a,rad_b);
			float hard_rad = (rad_a+rad_b) - 0.005*min(rad_a,rad_b);		
			...
			float temp = (-(m_a*m_b*G)+(q_a*q_b*e0))/d;			
...
//((1.0/6.0)*stuff[2]*cut_off*(fpe-fme-2*rad_a*rad_b*(pow(fpe,2)+pow(fme,2))))*(pow((d-hard_rad),2)/(cut_off-hard_rad) - (cut_off-hard_rad)) + (1.0/6.0)*stuff[2]*(2*rad_a*rad_b*(fpe+fme)-log(fpe)+log(fme));
			...
			float3 v_rel = v_temp; //(vtemp - p*Runit) - cross(wvec,Runit);				
			
			float dt = fmax((t_step-(d0-rad_a-rad_b)/p0)*step((d0-rad_a-rad_b)/p0,t_step)*step(0,(d0-rad_a-rad_b)/p0),0);
			
			float j = (F*t_step+0.5*dF*t_step*t_step)-m_a*m_b/(m_a+m_b)*(collisionflag*(stuff[1]*p0*(t_step-dt)+stuff[8]*(rad_a+rad_b-d0)*(t_step-dt)+0.5*stuff[8]*p0*(t_step-dt)*(t_step-dt))-(collisionflag-1)*(stuff[1]*p0*dt+0.5*stuff[8]*p0*dt*dt));
				
			float jf = collisionflag*length(v_rel)/((pow(rad_a,2)/I[a]+pow(rad_b,2)/I[b])+(1/m_a+1/m_b));
			
			float fdyn = (F*t_step + 0.5*dF*t_step*t_step);
