...
__kernel void Fimp(__global const double *q,			
		...
		{ 			
			...
			double Vpart = (-(m_a*m_b*G)+(q_a*q_b*e0))/d;			
			...
			if (d > cut_off) 
			{
				Vpart -= (1.0/6.0)*stuff[2]*(2*rad_a*rad_b*(fplus+fminus)-log(fplus)+log(fminus));
			...
			{
				Vpart -= ((1.0/6.0)*stuff[2]*(cut_off*(fpe-fme-2*rad_a*rad_b*(pow(fpe,2)+pow(fme,2)))*pow((d-hard_rad),2)/(cut_off-hard_rad) + (2*rad_a*rad_b*(fpe+fme)-log(fpe)+log(fme)) - cut_off*(fpe-fme-2*rad_a*rad_b*(pow(fpe,2)+pow(fme,2)))*(cut_off-hard_rad)));
//((1.0/6.0)*stuff[2]*cut_off*(fpe-fme-2*rad_a*rad_b*(pow(fpe,2)+pow(fme,2))))*(pow((d-hard_rad),2)/(cut_off-hard_rad) - (cut_off-hard_rad)) + (1.0/6.0)*stuff[2]*(2*rad_a*rad_b*(fpe+fme)-log(fpe)+log(fme));
			...
			while(atomic_cmpxchg(&flag[a], 0, 1) == 1) {};			
			accel[a] -= (j*Runit - collisionflag*step(0,jf)*jf*normalize(v_rel))/m[a]; 
			alpha[a] += cross(Runit,step(0,jf)*jf*v_rel)*rad[a]/I[a];
			V[a] += 0.5*Vpart; 
			Int[a] += 0.25*collisionflag*m_a*m_b/(m_a+m_b)*stuff[8]*pow((rad_a+rad_b-d0),2);			
			atomic_xchg(&flag[a],0);
			
			while(atomic_cmpxchg(&flag[b], 0, 1) == 1) {};
			accel[b] += (j*Runit - collisionflag*step(0,jf)*jf*normalize(v_rel))/m[b]; 
			alpha[b] += cross(Runit,step(0,jf)*jf*v_rel)*rad[a]/I[a];			
			V[b] += 0.5*Vpart;			
			Int[b] += 0.25*collisionflag*m_a*m_b/(m_a+m_b)*stuff[8]*pow((rad_a+rad_b-d0),2);
			atomic_xchg(&flag[b],0);				
		}
