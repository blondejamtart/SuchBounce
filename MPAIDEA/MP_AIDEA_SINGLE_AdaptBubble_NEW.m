function [memories_out,all_elements,options] = MP_AIDEA_SINGLE_AdaptBubble_NEW(fname,pop,vlb,vub,options,varargin)
%
% =========================================================================
...
% options(4)
if NP < size(pop,1)
    disp('ERROR: The input population has a number of elements greater than the expected value (defined in options(4))')
    disp(' ')
    return
end

% Neighborhood limit for GLOBAL restart
expstepglo = options(12);       
...
% Limit on delta f for CR update
dd_limit   = options(35);       

% Maximum number of function evaluations - NEW
nFeValMax  = options(38);

% DE strategy
DE_strategy = options(40);
                    ...
                    if sum(nFeVal)>= nFeValMax
                        for i_population = 1 : pop_number 
                            ...
                            if ~isempty(archivebest)
                                all_elements = [all_elements; memories(:,:,i_population); archivebest(:,1:D+1,i_population)];
                            else
                                all_elements = [all_elements; memories(:,:,i_population)];
                            end
                            all_elements = sortrows(all_elements,D+1);
                        end
                        sum(nFeVal)
                        return
                    else
...
for i_pop_number = 1 : pop_number  
   ...
   if sum(nFeVal) >= record(step)*nFeValMax   
       for i_pop_number = 1 : pop_number
           ...
           if ~isempty(archivebest)
               all_elements = [all_elements; memories(:,:,i_pop_number); archivebest(:,1:D+1,i_pop_number)];
           else
               all_elements = [all_elements; memories(:,:,i_pop_number)];
           end
           all_elements = sortrows(all_elements,D+1);
       end
       if sum(nFeVal)>= nFeValMax
...
while matrix_bubble_local == 0
    ...
    delta     = (max_minima_distance - min_minima_distance)/(D);
...
for i_pop_number = 1 : pop_number   
   ...
   if iglob(1,i_pop_number) == min(iglob)   
        ...
        if ~exist('warning','var') || (exist('warning','var') && warning(i_pop_number) ~= 1)
