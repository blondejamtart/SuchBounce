...
const F_kernel = "
		...
		{ 			
			...
			float j =  -(1 + stuff[2])*collisionflag*p/massrecip[x];					
					
			v2 += F*Runit*stuff[1]/m[lookup[x,2]] + dF*Runit*stuff[1]*stuff[1]/m[lookup[x,2]] - j*Runit/m[lookup[x,2]] - jf*v_rel/m[lookup[x,2]];
			v1 += -F*Runit*stuff[1]/m[lookup[x,1]] - dF*Runit*stuff[1]*stuff[1]/m[lookup[x,1]] + j*Runit/m[lookup[x,1]] + jf*v_rel/m[lookup[x,1]];
